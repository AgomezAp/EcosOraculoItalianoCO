  ...
  implements OnInit, OnDestroy, AfterViewChecked, AfterViewInit
{
  ...
  wheelPrizes: Prize[] = [
    ...
    // ✅ ELIMINATO: { id: '3', name: '2 Consulte Oniriche Extra', color: '#ffeaa7', icon: '🔮' },
    {
      id: '4',
      name: 'Riprova ancora!',
      color: '#ff7675',
  ...
  interpreterData: DreamInterpreterData = {
    name: 'Maestra Alma',
    specialty: 'Interpretazione dei Sogni e Simbologia Onirica',
    experience: "Secoli di interpretazione dei messaggi dell'inconscio",
  };

  // Frases de bienvenida aleatorias
  welcomeMessages = [
    "Ah, vedo che sei venuta a decifrare i misteri del tuo mondo onirico... I sogni sono finestre dell'anima. Raccontami, quali visioni ti hanno visitato?",
    "Le energie cosmiche mi sussurrano che hai sogni che richiedono interpretazione. Sono la Maestra Alma, Custode dei segreti onirici. Quale messaggio dell'inconscio ti preoccupa?",
    'Benvenuta, Viaggiatrice dei Sogni. I piani astrali mi hanno mostrato il tuo arrivo. Lasciami guidarti attraverso i simboli e i misteri delle tue visioni notturne.',
    'Il cristallo dei sogni brilla con la tua presenza... Sento che porti visioni che devono essere decifrate. Affidati alla mia saggezza antica e condividi i tuoi sogni con me.',
  ];
          ...
          setTimeout(() => {
            const successMessage: ConversationMessage = {
              ...
              message:
                ...
                '💭 Scopriamo insieme i segreti dei tuoi sogni!\n\n' +
                '📌 Nota: Questo pagamento è valido solo per il servizio di interpretazione dei sogni. Per altri servizi è richiesto un pagamento separato.',
              timestamp: new Date(),
            };
  ...
  onPrizeWon(prize: Prize): void {
    ...
    const prizeMessage: ConversationMessage = {
      role: 'interpreter',
      message: `🌙 Le energie cosmiche ti hanno benedetto! Hai vinto: **${prize.name}** ${prize.icon}\n\nQuesto dono dell\'universo onirico è stato attivato per te. I misteri dei sogni si rivelano a te con maggiore chiarezza. Che la fortuna ti accompagni nelle tue prossime interpretazioni!`,
      timestamp: new Date(),
    };
    ...
    switch (prize.id) {
      ...
      case '2': // 1 Análisis Premium - ACCESO COMPLETO
        ...
        const premiumMessage: ConversationMessage = {
          role: 'interpreter',
          message:
            "✨ **Hai sbloccato l'accesso Premium completo!** ✨\n\nI segreti del mondo dei sogni ti hanno sorriso in modo straordinario. Ora hai accesso illimitato a tutta la saggezza dei sogni. Puoi chiedermi interpretazioni, simboli onirici e tutti i segreti dell'inconscio in qualsiasi momento.\n\n🌙 *Le porte del regno dei sogni si sono aperte completamente per te* 🌙",
          timestamp: new Date(),
        };
      ...
      .subscribe({
        next: (response: any) => {
          ...
          if (response.success && response.response) {
            ...
            this.handleError(
              "Errore nel recupero della risposta dell'interprete"
            );
          }
        },
        error: (error: any) => {
  ...
  async handlePaymentSubmit(): Promise<void> {
    ...
    try {
      ...
      this.paymentError =
        error.message || 'Errore durante l\'inizializzazione del pagamento PayPal.';
      this.isProcessingPayment = false;
      this.cdr.markForCheck();
    }
  }
  ...
  private handleError(errorMessage: string): void {
    const errorMsg: ConversationMessage = {
      role: 'interpreter',
      message: `🔮 Le energie cosmiche sono disturbate... ${errorMessage} Riprova quando le vibrazioni si saranno stabilizzate.`,
      timestamp: new Date(),
    };
  ...
  onUserDataSubmitted(userData: any): void {
    ...
    if (missingFields.length > 0) {
      alert(
        `Per procedere con il pagamento, devi compilare i seguenti campi: ${missingFields.join(
          ', '
        )}`
      );
