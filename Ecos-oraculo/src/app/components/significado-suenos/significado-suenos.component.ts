  ...
  implements OnInit, OnDestroy, AfterViewChecked, AfterViewInit
{
  ...
  wheelPrizes: Prize[] = [
    {
      id: '1',
      name: '3 interpretazioni gratuite',
      color: '#4ecdc4',
      icon: '🌙',
    },
    {
      id: '2',
      name: '1 analisi premium dei sogni',
      color: '#45b7d1',
      icon: '✨',
    },
    {
  ...
  welcomeMessages = [
    "Ah, vedo che sei venuto a decifrare i misteri del tuo mondo onirico... I sogni sono finestre sull'anima. Raccontami, quali visioni ti hanno visitato?",
    'Le energie cosmiche mi sussurrano che hai sogni che devono essere interpretati. Sono la Maestra Alma, custode dei segreti onirici. Quale messaggio del subconscio ti preoccupa?',
    'Benvenuto, viaggiatore dei sogni. I piani astrali mi hanno mostrato il tuo arrivo. Lasciami guidarti attraverso i simboli e i misteri delle tue visioni notturne.',
    'Il cristallo dei sogni brilla con la tua presenza... Sento che porti visioni che devono essere decifrate. Affidati alla mia antica saggezza e condividi i tuoi sogni con me.',
  ];
        ...
        if (verification.valid && verification.status === 'approved') {
          ...
          setTimeout(() => {
            const successMessage: ConversationMessage = {
              ...
              message:
                ...
                "✨ Grazie mille per il tuo pagamento. Ora hai accesso completo all'interpretazione dei sogni.\n\n" +
                '💭 Andiamo insieme a scoprire i segreti dei tuoi sogni!\n\n' +
                '📌 Nota: Questo pagamento è solo per il servizio di interpretazione dei sogni.',
              timestamp: new Date(),
            };
          ...
          setTimeout(() => {
            const errorMessage: ConversationMessage = {
              role: 'interpreter',
              message:
                '❌ Non è stato possibile verificare il pagamento. Per favore, riprova o contatta il nostro supporto se il problema persiste.',
              timestamp: new Date(),
            };
  ...
  onPrizeWon(prize: Prize): void {
    const prizeMessage: ConversationMessage = {
      role: 'interpreter',
      message: `🌙 Le energie cosmiche ti hanno benedetto! Hai vinto: **${prize.name}** ${prize.icon}\n\nQuesto regalo dell'universo onirico è stato attivato per te. I misteri dei sogni si riveleranno con maggiore chiarezza. Che la fortuna ti accompagni nelle tue prossime interpretazioni!`,
      timestamp: new Date(),
    ...
    switch (prize.id) {
      ...
      case '2': // 1 Análisis Premium - ACCESO COMPLETO
        ...
        const premiumMessage: ConversationMessage = {
          role: 'interpreter',
          message:
            "✨ **Hai sbloccato l'accesso Premium completo!** ✨\n\nI segreti del mondo onirico ti hanno sorriso in modo straordinario. Ora hai accesso illimitato a tutta la saggezza dei sogni. Puoi consultare interpretazioni, simboli onirici e tutti i segreti del subconscio tutte le volte che desideri.\n\n🌙 *Le porte del regno dei sogni si sono aperte completamente per te* 🌙",
          timestamp: new Date(),
        };
  ...
  private addFreeDreamConsultations(count: number): void {
    ...
    const infoMessage: ConversationMessage = {
      role: 'interpreter',
      message: `✨ *Hai ricevuto ${count} interpretazioni dei sogni gratuite* ✨\n\nOra hai **${newTotal}** consulte disponibili per esplorare i misteri dei tuoi sogni.`,
      timestamp: new Date(),
    };
  ...
  private useFreeConsultation(): void {
    ...
    if (freeConsultations > 0) {
      ...
      const prizeMsg: ConversationMessage = {
        role: 'interpreter',
        message: `✨ *Hai utilizzato un'interpretazione gratuita* ✨\n\nTi restano **${remaining}** interpretazioni gratuite disponibili.`,
        timestamp: new Date(),
      };
  ...
  sendMessage(): void {
    if (this.messageText?.trim() && !this.isLoading) {
      ...
      console.log(
        `📊 Sogni - Messaggio #${nextMessageCount}, Premium: ${this.hasUserPaidForDreams}, Limite: ${this.FREE_MESSAGES_LIMIT}`
      );
      ...
      .subscribe({
        next: (response: DreamChatResponse) => {
          ...
          if (response.success && response.response) {
            ...
            this.handleError(
              response.error ||
                "Errore nell'ottenere la risposta dell'interprete"
            );
          }
        },
        error: (error: any) => {
  ...
  async handlePaymentSubmit(): Promise<void> {
    ...
    try {
      await this.paypalService.initiatePayment({
        amount: '4.00',
        currency: 'EUR',
        serviceName: 'Significato dei Sogni',
        returnPath: '/significato-sogni',
        cancelPath: '/significato-sogni',
      });
    } catch (error: any) {
      this.paymentError =
  ...
  private handleError(errorMessage: string): void {
    const errorMsg: ConversationMessage = {
      role: 'interpreter',
      message: `🔮 Le energie cosmiche sono perturbate... ${errorMessage} Riprova quando le vibrazioni si saranno stabilizzate.`,
      timestamp: new Date(),
    };
  ...
  onUserDataSubmitted(userData: any): void {
    ...
    if (missingFields.length > 0) {
      alert(
        `Per continuare con il pagamento, devi completare i seguenti campi: ${missingFields.join(
          ', '
        )}`
      );
      this.showDataModal = true;
    ...
    try {
      ...
      console.error('Errore nel salvare userData:', error);
    }
  ...
  getPrizesAvailable(): string {
    ...
    if (freeConsultations > 0) {
      prizes.push(
        `${freeConsultations} interpretazion${
          freeConsultations > 1 ? 'i' : 'e'
        } gratis`
      );
    }

    return prizes.length > 0 ? prizes.join(', ') : 'Nessuno';
  }
}
