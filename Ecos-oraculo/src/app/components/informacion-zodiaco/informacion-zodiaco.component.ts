  ...
  implements OnInit, OnDestroy, AfterViewChecked
{
  ...
  astralPrizes: Prize[] = [
    {
      id: '1',
      name: '3 Lanci della Ruota Astrale',
      color: '#4ecdc4',
      icon: '🔮',
    },
    {
  ...
  astrologerInfo = {
    ...
    specialty: 'Specialista di Astrologia e Segni Zodiacali',
  };
    ...
    switch (prize.id) {
      ...
      case '2': // 1 Lectura Premium - ACCESO COMPLETO
        ...
        const premiumMessage = {
          isUser: false,
          content:
            "✨ **Hai sbloccato l'accesso Premium completo!** ✨\n\nLe stelle hanno cospitato in modo straordinario per aiutarti. Ora hai accesso illimitato a tutta la conoscenza astrale. Puoi consultarmi sui segni zodiacali, compatibilità, previsioni astrologiche e tutti i segreti celesti tutte le volte che desideri.\n\n🌟 *Le stelle hanno aperto tutte le loro porte cosmiche per te* 🌟",
          timestamp: new Date(),
        };
  ...
  private generateAstrologyResponse(userMessage: string): Observable<string> {
    ...
    return this.zodiacoService.chatWithAstrologer(request).pipe(
      ...
      catchError((error: any) => {
        return of(
          '🌟 Le stelle sono temporaneamente nuvolose. Le stelle mi sussurrano che devo ricaricare le mie energie cosmiche. Per favore riprova tra qualche istante.'
        );
      })
    );
  }
  ...
  async handlePaymentSubmit(): Promise<void> {
    ...
    try {
      await this.paypalService.initiatePayment({
        amount: '5.00',
        currency: 'EUR',
        serviceName: 'Zodiaco Informazione',
