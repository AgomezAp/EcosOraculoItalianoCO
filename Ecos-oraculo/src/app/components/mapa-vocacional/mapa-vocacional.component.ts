  ...
  implements OnInit, OnDestroy, AfterViewChecked, AfterViewInit
{
  ...
  counselorInfo = {
    ...
    specialty: 'Orientamento professionale e mappe di carriera personalizzate',
  };
  ...
  vocationalPrizes: Prize[] = [
    ...
    {
      id: '2',
      name: '1 Analisi Premium di Carriera',
      color: '#45b7d1',
      icon: '✨',
    },
    {
  ...
  async ngOnInit(): Promise<void> {
          ...
          setTimeout(() => {
            this.addMessage({
              ...
              content:
                ...
                '✨ Grazie per il tuo pagamento. Ora hai accesso completo alla Mappa Vocazionale.\n\n' +
                '💼 Scopriamo insieme il tuo futuro professionale!\n\n' +
                '📌 Nota: Questo pagamento è valido solo per il servizio di Mappa Vocazionale.',
              timestamp: new Date(),
              isUser: false,
            });
    ...
    if (savedMessages) {
      try {
        ...
        console.error('Errore nel parsing dei messaggi:', error);
      }
    }
  ...
  initializeWelcomeMessage(): void {
    this.addMessage({
      sender: this.counselorInfo.name,
      content: `Ciao! Sono ${this.counselorInfo.name}, la tua specialista in Orientamento Professionale. Sono qui per aiutarti a scoprire la tua vera vocazione e progettare una mappa di carriera personalizzata per te.`,
      timestamp: new Date(),
      isUser: false,
    });
  ...
  sendMessage(): void {
    ...
    console.log(
      `📊 Vocazionale - Messaggio #${nextMessageCount}, Premium: ${this.hasUserPaidForVocational}, Limite: ${this.FREE_MESSAGES_LIMIT}`
    );
        ...
        next: (response: VocationalResponse) => {
          ...
          if (response.success && response.response) {
            ...
            this.addMessage({
              ...
              content:
                response.error ||
                'Mi dispiace, sto riscontrando difficoltà tecniche. Potresti riformulare la tua domanda?',
              timestamp: new Date(),
              isUser: false,
            });
            this.saveMessagesToSession();
  ...
  onPrizeWon(prize: Prize): void {
    const prizeMessage: ChatMessage = {
      sender: this.counselorInfo.name,
      content: `🎯 Eccellente! Il destino professionale ti ha benedetto. Hai vinto: **${prize.name}** ${prize.icon}\n\nQuesto dono dell'universo professionale è stato attivato per te. Le opportunità di carriera si allineano a tuo favore. Che questa fortuna ti guidi verso la tua vera vocazione!`,
      timestamp: new Date(),
      isUser: false,
    ...
    switch (prize.id) {
      ...
      case '2': // 1 Análisis Premium - ACCESO COMPLETO
        ...
        const premiumMessage: ChatMessage = {
          sender: this.counselorInfo.name,
          content:
            "✨ **Hai sbloccato l'accesso Premium completo!** ✨\n\nIl destino professionale ti ha sorriso in modo straordinario. Ora hai accesso illimitato a tutta la mia esperienza in orientamento professionale. Puoi consultare sulla tua vocazione, valutazioni di carriera e tutti gli aspetti del tuo futuro professionale tutte le volte che desideri.\n\n🎯 *Le porte del tuo percorso professionale si sono aperte completamente* 🎯",
          timestamp: new Date(),
          isUser: false,
        };
  ...
  private addFreeVocationalConsultations(count: number): void {
    ...
    const infoMessage: ChatMessage = {
      sender: this.counselorInfo.name,
      content: `✨ *Hai ricevuto ${count} consultazioni vocazionali gratuite* ✨\n\nOra hai **${newTotal}** consultazioni disponibili per esplorare il tuo futuro professionale.`,
      timestamp: new Date(),
      isUser: false,
    };
  ...
  async handlePaymentSubmit(): Promise<void> {
    ...
    try {
      const orderData = {
        ...
        returnPath: '/mappa-vocazionale',
        cancelPath: '/mappa-vocazionale',
      };
  ...
  savePersonalInfo(): void {
    ...
    if (Object.keys(this.personalInfo).length > 0) {
      this.addMessage({
        sender: this.counselorInfo.name,
        content: `Perfetto, ho registrato le tue informazioni personali. Questo mi aiuterà a fornirti un orientamento più preciso e personalizzato. C'è qualcosa di specifico sul tuo futuro professionale che ti preoccupa o ti entusiasma?`,
        timestamp: new Date(),
        isUser: false,
      });
    }
  ...
  onUserDataSubmitted(userData: any): void {
    ...
    if (missingFields.length > 0) {
      alert(
        `Per procedere con il pagamento, devi completare i seguenti campi: ${missingFields.join(
          ', '
        )}`
      );
      this.showDataModal = true;
